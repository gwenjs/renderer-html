 ...
 * HTMLPlugin.onRender() → service.applyWorldTransforms() → HTMLLayer.applyTransform() → DOM
 */
import { describe, it, expect } from 'vitest'
import { HTMLRenderer } from '../src/html-renderer-service.js'

describe('camera integration — applyWorldTransforms call chain', () => {
  it('world-space entity stays correct as camera moves', () => {
    ...
    service.applyWorldTransforms(100, 75, 1, { x: 0, y: 0, width: 1, height: 1 })
  ...
  it('zoom scales down world-space elements', () => {
    ...
    service.applyWorldTransforms(0, 0, 2, { x: 0, y: 0, width: 1, height: 1 })
    const inner = (service.getLayerElement('world')).firstElementChild as HTMLElement
  ...
  it('HUD screen layer is never affected by camera transforms', () => {
    ...
    service.resize(800, 600)
    service.applyWorldTransforms(999, 999, 1, { x: 0, y: 0, width: 1, height: 1 })
